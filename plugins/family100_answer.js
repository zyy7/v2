...
`: ''}
${isWin ? `*SEMUA JAWABAN TERJAWAB*` : isSurrender ? '*𝙈𝙚𝙣𝙮𝙚𝙧𝙖𝙝!!*' : ''}
...
${isSurrender ? '' : `+${room.winScore} XP tiap jawaban benar`}
    `.trim()
        await this.sendButton(m.chat, caption, watermark, `${isWin ? 'Family 100' : isSurrender ? 'Family 100' : '𝖭𝗒𝖾𝗋𝖺𝗁'}`, `${isWin ? ',family100' : isSurrender ? ',family100' : 'nyerah'}`, m, {
            contextInfo: {
